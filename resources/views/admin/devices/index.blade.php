            ...
            <x-admin.grid.table>
                <x-slot name="head">
                    <tr class="bg-base-200">
                        ...
                        @canany(['deviceUpdate', 'deviceDelete', 'deviceEditMedia', 'deviceUpdateOrder'], new \App\Models\Device)
                        <x-admin.grid.th>
                            {{ __('Actions') }}
                        </x-admin.grid.th>
                        @endcanany
                        ...
                        <x-admin.grid.td>
                            <form action="{{ route('admin.devices.destroy', $device->id) }}" method="POST">
                                <div>
                                    ...
                                    <a  title="Update Device" href="{{route('admin.devices.edit', $device->id)}}" class="btn btn-square btn-ghost">
                                    ...
                                    <a title="Assign/Update Media" href="{{route('admin.devices.editMedia', $device->id)}}" class="btn btn-square btn-ghost">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" />
                                          </svg>                                          
                                    </a>
                                    @endcan
                                    @can('deviceUpdateOrder', $device)
                                    <a title="Arrange Media" href="{{route('admin.devices.media.index', $device->id)}}" class="btn btn-square btn-ghost">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button title="Delete" class="btn btn-square btn-ghost" onclick="return confirm('{{ __('Are you sure you want to delete?') }}')">
